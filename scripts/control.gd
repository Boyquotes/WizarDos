...
func _fixed_process(delta):
	...
	if (finished):
		get_node("background/window/announcment").set_text(winner+" wins!")
		if (winner == "Blue"):
			get_node("background/window/announcment").add_color_override("font_color", Color(0.22,0.26,0.91,1))
		else:
			get_node("background/window/announcment").add_color_override("font_color", Color(0.96,0.3,0.3,1))
